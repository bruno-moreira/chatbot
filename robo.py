...
def configurar():
    ...
    robo = ChatBot("Robô de informações sobre zabbix & grafana", read_only = True, logic_adapters = [{"import_path": "chatterbot.logic.BestMatch"}])
...
def executar(robo):
    while True:
        ...
        if resposta.confidence >= CONFIANCA_MINIMA:
            print(f"Serviços >> {resposta.text} [confiança={resposta.confidence}]")
        else:
